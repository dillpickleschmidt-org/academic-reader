...
function extractTextFromRect(
  ...
  for (const { c, y } of chars) {
    if (lastY !== null && Math.abs(y - lastY) > 5) result += " "
    result += c
    lastY = y
  }
  return result
...
function findTextOnPage(
  ...
  for (let i = clampedStart; i < elements.length; i++) {
    ...
    if (findWithBoundaryCheck(text, normalizedSearch, isShortNum) !== -1) {
...
function similarity(candidate: string, target: string): number {
  ...
  return longestCommonSubsequenceLength(candidate, target) / target.length
}
