  ...
  return (
    <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center">
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="text-[7px] uppercase tracking-widest text-muted-foreground/50 font-mono">
          PDF on mobile
        </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <div className="space-y-[2px]">
                {[...Array(8)].map((_, j) => (
                  <div
                    key={j}
                    className="h-[1.5px] bg-muted-foreground/12 rounded"
                    style={{ width: `${65 + ((j * 13) % 35)}%` }}
                  />
                ))}
              </div>
              <div className="space-y-[2px]">
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="text-[7px] uppercase tracking-widest text-primary/50 font-mono">
          Converted
        </div>
          ...
          <div className="rounded-lg bg-background p-2 space-y-2 flex-1 overflow-hidden">
            ...
            <div className="space-y-1">
              {[...Array(5)].map((_, j) => (
                <div
                  key={j}
                  className="h-[2.5px] bg-foreground/10 rounded"
                  style={{ width: `${80 + ((j * 11) % 20)}%` }}
                />
              ))}
            </div>
